...
class LogUseCase {
  ...
  Future<void> _checkAndAwardBadges(User user) async {
    ...
    final totalLogs = await _logRepository.getTotalLogsCount(user.id);
    ...
    if (totalLogs >= 7) await _badgeRepository.earnBadge('log_7');
    if (totalLogs >= 30) await _badgeRepository.earnBadge('log_30');
  }
  ...
  Future<List<DailyLogEntry>> getLogsByDateRange(
    ...
    final user = await _userRepository.getCurrentUser();
    if (user == null) return [];
    return _logRepository.getLogsByDateRange(user.id, start, end);
  }
}